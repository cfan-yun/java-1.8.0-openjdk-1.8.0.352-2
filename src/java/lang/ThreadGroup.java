...
class ThreadGroup implements Thread.UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread t, Throwable e) {
        if (parent != null) {
            parent.uncaughtException(t, e);//这是个递归调用，先调用父线程组的异常处理方法
        } else {
            Thread.UncaughtExceptionHandler ueh =
                Thread.getDefaultUncaughtExceptionHandler();//调用线程默认的处理异常的Handler ,即通过Thread.setDefualtUncaughtExectionHandler设置的handler
            if (ueh != null) {
                ueh.uncaughtException(t, e);
            } else if (!(e instanceof ThreadDeath)) {//没有配置Handler ,直接打印到控制台。
